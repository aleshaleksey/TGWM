...
// "MooseMatrix" and "MooseElement", which supports setting Ids to AdvWidgetCycler..
...
impl Widget for MooseMatrix {
    type State = State;
    type Style = Style;
    type Event = MooseElements;
    ...
    fn update(self, args: widget::UpdateArgs<Self>) -> Self::Event {
        ...
        let MooseMatrix { cols, rows, .. } = self;
...
impl MooseElements {
    ...
    pub fn next(&mut self, ui: &Ui) -> Option<MooseElement> {
        let MooseElements {
            ref mut row,
            ref mut col,
            num_rows,
        ...
        let widget_id = match ui.widget_graph().widget(matrix_id)
            .and_then(|container| container.unique_widget_state::<MooseMatrix>())
...
impl MooseElement {
    ...
    pub fn set<W>(self,
				  widget: W,
				  ui: &mut UiCell,
				  awc:& Mutex<cmoose::AdvWidgetCycler>) -> W::Event
        where W: Widget,
    {
